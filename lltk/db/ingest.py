    ...
    with logmap('Ingesting freqs from JSONs...') as log:
        if truncate_first:
            ...
            if corpora:
                ...
                log.debug(f'Deleted existing rows for corpora: {sorted(corpora)}')
            else:
                ch_adapter.execute('TRUNCATE TABLE lltk.text_freqs')
                log.debug('Truncated lltk.text_freqs')
        ...
        if not todo:
            log.debug('Nothing to do — all texts already ingested (or no path_freqs)')
            return 0
        ...
        log.debug(f'{len(todo):,} freqs JSONs to ingest '
        ...
        log.debug(f'lltk.text_freqs now has {n:,} rows total')
        return n_inserted
