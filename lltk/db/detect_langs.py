...
def detect_langs_clickhouse(ch_adapter, min_tokens=50,
    ...
    """
    with logmap('Detecting languages...') as log:
        from lltk.tools.lang_detect import function_words_table

        with logmap('Loading stopwords...') as sw_log:
            ch_adapter.execute("""
            ...
            sw_log.debug(f'{len(langs)} languages ({n_total} words total)')
        ...
        if skip_existing:
            ...
            if already:
                log.debug(f'Skipping {already:,} already-processed texts')
        ...
        with logmap(f'Processing {n_remaining:,} texts...') as proc_log:
            if n_remaining == 0:
                proc_log.debug('Nothing to process')
                return {}
            ...
            t0 = time.time()
            proc_log.debug('Running server-side INSERT...')
            try:
            ...
            proc_log.debug(f'Done in {elapsed:.1f}s ({n_remaining/max(elapsed, 1):.0f}/s)')

        with logmap('Language distribution...') as dist_log:
