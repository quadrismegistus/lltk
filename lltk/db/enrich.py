...
def enrich_genres_ch(ch_adapter, progress=True):
    ...
    with logmap('Enriching genres...') as log:
        ...
        with logmap('Gathering authority match groups...') as ag_log:
            ...
            if not len(auth_groups):
                ag_log.debug('No authority-containing match groups found')
        ...
        log.debug(f'{len(auth_groups):,} match groups have authority members')
        ...
        with logmap(f'Writing {len(override_rows):,} override rows...') as ov_log:
        ...
        # corpus labels.
        with logmap('Syncing back to lltk.texts...') as sync_log:
            ch_adapter.execute("""
        ...
        stats = ch_adapter.query_df("""
            ...
            ORDER BY n DESC
        """)
        log.debug(f'Genre enrichment source distribution:\n{stats.to_string(index=False)}')
        return stats
...
def detect_translations_ch(ch_adapter):
    ...
    """
    with logmap('Detecting translations...') as log:

        # One query: get group_id, _id, lang, year for groups with multiple langs
        with logmap('Gathering cross-language match groups...') as cl_log:
        ...
        if not len(cross_lang):
            log.debug('No cross-language match groups found')
            return {}
        ...
        log.debug(f'{len(translations):,} texts marked as translations')
        ...
        log.debug(f'Top translation flows (original_lang -> translated):\n{flow_counts.head(10).to_string(index=False)}')
        return flow_counts
