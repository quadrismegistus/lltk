...
def download(url,save_to,overwrite=False,**attrs):
    ...
    return download_file(url,save_to,**attrs)

def download_file(url, filename, desc='', **attrs):
    """
    ...
    total=int( r.headers['Content-Length'] ) / 1024 / 1024
    if desc: desc+=f' ({int(round(total,0))}MB)'
    with open(filename, 'wb') as f:
        pbar = tqdm(unit="MB", total=total, unit_scale=True, desc=desc, **attrs)
...
def download_tqdm(url, save_to,desc=''):
    import requests
    from tqdm import tqdm
    ...
    with open(save_to, 'wb') as f:
        for chunk in tqdm(r.iter_content(32*1024), total=total_size, unit='B',unit_scale=True, desc=desc):
            if chunk:
                f.write(chunk)

    return save_to

def unzip(zipfn, dest='.', flatten=False, overwrite=False, replace_in_filenames={}, progress=True, progress_desc='Extracting zip file'):
    ...
    with ZipFile(zipfn) as zip_file:
        ...
        # Loop over each file
        iterr=tqdm(iterable=namelist, total=len(namelist), desc=progress_desc) if progress else namelist
        for member in iterr:
...
def untar(fname,dest='.',overwrite=False,progress=True,progress_desc='Extracting tar file',**attrs):
    import tarfile
    from tqdm import tqdm
...
def pmap_iter(func, objs, num_proc=4, use_threads=False, desc=''):
    """
    ...
    if num_proc>1:
        # create pool
        if desc: desc+=f' (nproc={num_proc})'
        pool=mp.Pool(num_proc) if not use_threads else mp.ThreadPool(num_proc)

        # yield iter
        for res in tqdm(pool.imap_unordered(func, objs),total=len(objs),desc=desc):
            yield res
    else:
        # yield
