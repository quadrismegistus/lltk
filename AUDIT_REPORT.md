...
### ✅ Done 2026-07-05
- **PR #10 merged** to master (all llm2 features + the fixes above); CI green on 3.9/3.11/3.12. Merged branches pruned (app2, claude, clickhouse-migration, spring-cleaning, llm2).
- **Dead-file cleanup** — removed `db/migrate.py`, `scripts/minhash_match.py`, stale notebooks, `manifest-test.txt`, and the 58 MB `web/data/data.fields.pickle` (git rm only; history NOT rewritten — a `filter-repo` pass to shrink the 141 MB `.git` is still an open option).
- **Docs** — version → 0.10.0, test count → 762, `requires-python` → `>=3.9` + version classifiers; CLAUDE.md MinHash section repointed to `lltk/db/minhash.py`.
- **H2 FIXED** (PR #11, awaiting review) — curated-corpus whitelist now uses `fetch_metadata`; match-group propagation is a ClickHouse `match_groups` tmp-table self-join. Verified against real data + end-to-end (`arc_fiction.load_metadata()` → 86,469 rows).

### ⏭️ Remaining — highest priority (NOT done)
1. **Branch consolidation — DECISION NEEDED.** `no-stars` (13 commits, ~171-file conflicts) and `spring-cleaning2` (27 commits, ~62-file conflicts incl. rename/delete) both substantially OVERLAP refactors master already did (star-import removal; `tools/clickhouse_*` → `db/*` move). Recommend NOT wholesale-merging: audit each for still-unique value, cherry-pick that, then delete the rest. Needs maintainer sign-off on abandoning the remainder.
